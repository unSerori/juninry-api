...
type RewardExchanging struct {
	...
	RewardUuid   string    `xorm:"varchar(36)" json:"rewardUUID" form:"rewardUUID"` // ユーザーID
	UserUuid     string    `xorm:"varchar(36)" json:"userUUID"`                     // クラスID
	ExchangingAt time.Time `xorm:"DATETIME not null" json:"exchangingAt"`                           // 画像ファイル名一覧 // TEXT型でUTF-8 21,845文字 // 一画像40文字と考えると最大546.125画像保存可能
...
// 交換記録を登録
func RewardExchange(reward RewardExchanging) (bool, error) {
...
// 交換記録の処理を更新　０→１に
func UpdateRewardExchanging(rewardExchangingId string) (bool, error) {
	affected, err := db.Where("reward_exchanging_id = ?", rewardExchangingId).Update(&RewardExchanging{Exchange: 1})
	if err != nil || affected == 0 {
		return false, err
	}
	return true, err
